...
class _PlantioEstandePlantasScreenState extends State<PlantioEstandePlantasScreen> {
  ...
  /// Busca dados de CV% existentes para comparação agronômica
  Future<void> _buscarDadosCVExistentes() async {
    if (_talhaoSelecionado == null) return;
    
    try {
      Logger.info('🔍 Buscando dados de CV% existentes para comparação...');
      
      // Aqui você implementaria a busca no banco de dados
      // Por enquanto, vou simular dados para demonstração
      // Em produção, você buscaria dados reais de CV% já calculados
      
      // Simular dados de CV% existentes (em produção, buscar do banco)
      _dadosCVExistentes = {
        'cvPercentual': 12.5, // CV% da calibração
        'plantasPorMetro': 8.5, // Plantas/m esperadas da calibração
        'populacaoHectare': 34000, // População/ha esperada
        'dataCalibracao': '2024-01-15',
        'classificacao': 'EXCELENTE',
      };
      
      _cvEsperado = _dadosCVExistentes!['cvPercentual'];
      _plantasPorMetroEsperado = _dadosCVExistentes!['plantasPorMetro'];
      _populacaoEsperada = _dadosCVExistentes!['populacaoHectare'];
      
      Logger.info('✅ Dados de CV% encontrados:');
      ...
      Logger.error('❌ Erro ao buscar dados de CV%: $e');
    }
  }
...
CV% Real: ${_coeficienteVariacao?.toStringAsFixed(1)}%
${diferencaCV != null ? 'Diferença CV%: ${diferencaCV > 0 ? '+' : ''}${diferencaCV.toStringAsFixed(1)}%' : ''}
...
''';
  ...
  Future<void> _calcular() async {
    ...
    try {
      ...
      final espacamentoEntrePlantasCm = double.tryParse(_espacamentoController.text.replaceAll(',', '.')) ?? 0;
      final areaMedidaM2 = double.tryParse(_metrosQuadradosController.text.replaceAll(',', '.')) ?? 0;
      final sementesPlantadasArea = double.tryParse(_plantasContadasController.text.replaceAll(',', '.')) ?? 0;
      ...
      // Validação dos campos obrigatórios (CORREÇÃO: Validação mais flexível)
      ...
      final espacamentoEntrePlantasM = espacamentoEntrePlantasCm / 100;
      ...
      // 📈 CÁLCULOS DE EFICIÊNCIA E COMPARAÇÃO
      double? eficiencia;
      if (populacaoIdealEsperada > 0) {
        eficiencia = (plantasPorHectareFinal / populacaoIdealEsperada) * 100;
      }
      
      // 🌱 CÁLCULOS DE SEMENTES (se dados disponíveis)
      double? sementesPorMetroReal;
      double? sementesPorMetroEsperado;
      double? porcentagemVariacaoSementes;
      double? populacaoIdealCalculada;
      double? populacaoEsperada;
      double? porcentagemVariacaoPopulacao;
      
      // Sementes por metro real (CORREÇÃO: fórmula agronômica correta)
      if (sementesPlantadasArea > 0 && areaMedidaM2 > 0 && distanciaEntreLinhasM > 0) {
        ...
        if (sementesPorMetroDesejado > 0) {
          sementesPorMetroEsperado = sementesPorMetroDesejado;
          porcentagemVariacaoSementes = ((sementesPorMetroReal - sementesPorMetroEsperado) / sementesPorMetroEsperado) * 100;
        }
      }
      
      // População ideal calculada (CORREÇÃO: fórmula agronômica correta)
      if (espacamentoEntrePlantasCm > 0 && distanciaEntreLinhasCm > 0) {
        // FÓRMULA AGRONÔMICA CORRETA: 
        // População/ha = (10000 m²/ha) ÷ (espaçamento_entre_linhas_m × espaçamento_entre_plantas_m)
        final espacamentoLinhasM = distanciaEntreLinhasCm / 100;
  ...
  void _limpar() {
    setState(() {
      ...
      _plantasContadasController.clear();
      _espacamentoController.clear();
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    ...
                    'ABORDAGEM 2 - Contagem por Linha (m linear):\n'
                    '• Informe espaçamento entre plantas na linha\n'
                    '• Sistema calcula plantas por metro linear\n'
                    '• Converte para população por hectare',
                    style: TextStyle(fontSize: 11),
                  ),
                ],
              ),
            ...
            if (!_usarMultiplasLinhas)
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Plantas contadas na área (ABORDAGEM 1)',
                  hintText: 'Ex: 120',
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Espaçamento entre plantas (ABORDAGEM 2)',
                hintText: 'Ex: 25',
                helperText: 'Distância entre plantas na mesma linha (para cálculo por metro linear)',
                suffixText: 'cm',
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'População (sementes)',
                hintText: 'Ex: 100',
                helperText: 'Número de sementes plantadas na área medida',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Resultados de população
            if (_populacaoIdealCalculada != null || _populacaoEsperada != null) ...[
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Calculada:'),
                        Text(
                          _populacaoIdealCalculada != null
                              ? NumberFormat('#,###').format(_populacaoIdealCalculada)
                              : '-',
                          style: const TextStyle(fontWeight: FontWeight.bold),
                        ),
                      ],
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          _populacaoEsperada != null
                              ? NumberFormat('#,###').format(_populacaoEsperada)
                              : '-',
                          style: const TextStyle(fontWeight: FontWeight.bold),
                        ),
                      ],
                    ),
                    if (_porcentagemVariacaoPopulacao != null) ...[
                      const SizedBox(height: 8),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('Variação:'),
                          Text(
                            '${_porcentagemVariacaoPopulacao! > 0 ? '+' : ''}${NumberFormat('0.00').format(_porcentagemVariacaoPopulacao)}%',
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: _porcentagemVariacaoPopulacao!.abs() <= 10 
                                  ? Colors.green 
                                  : _porcentagemVariacaoPopulacao!.abs() <= 20 
                                      ? Colors.orange 
                                      : Colors.red,
                            ),
                          ),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // CV% Esperado vs Real
                    if (_cvEsperado != null && _coeficienteVariacao != null) ...[
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('CV% Real (Estande):'),
                          Text(
                            '${_coeficienteVariacao!.toStringAsFixed(1)}%',
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
  ...
  Future<void> _salvar() async {
    ...
    try {
      ...
      final estande = EstandePlantasModel.novo(
        ...
        plantasContadas: int.tryParse(_plantasContadasController.text) ?? 0,
      ...
      if (_usarMultiplasLinhas && _mediaPlantasPorLinha != null && _coeficienteVariacao != null) {
        ...
        // Salvar o CV% no banco de dados
        print('🔍 Salvando CV% do plantio...');
        print('📊 Dados do CV%: talhaoId=${_plantingCVModel!.talhaoId}, cv=${_plantingCVModel!.coeficienteVariacao}%');
        await _plantingCVRepository.salvar(_plantingCVModel!);
        print('✅ CV% salvo com sucesso!');
      }
      ...
      final calibracaoHistorico = CalibrationHistoryModel(
        ...
        linhasColetadas: int.tryParse(_plantasContadasController.text),
  ...
  String _getCulturaIdFromName(String culturaName) {
    if (culturaName.trim().isEmpty) {
      return '1'; // ID padrão se nome estiver vazio
    }
    
    final culturaLower = culturaName.toLowerCase().trim();
    
    // Mapear nomes de culturas para IDs da tabela culturas
    final culturaMap = {
      'soja': '1',
      'milho': '2',
      'trigo': '3',
    ...
    // Buscar no mapa de culturas
    final culturaId = culturaMap[culturaLower];
    if (culturaId != null) {
      print('🔍 Cultura "$culturaName" mapeada para ID "$culturaId"');
      return culturaId;
    }
    
    // Fallback: buscar nas culturas já carregadas
    ...
    // Converter ID da cultura para ID da tabela culturas
    final culturaIdFromMap = culturaMap[culturaEncontrada.name.toLowerCase()] ?? '1';
    
    print('🔍 Buscando cultura "$culturaName": encontrada "${culturaEncontrada.name}" mapeada para ID "$culturaIdFromMap"');
    return culturaIdFromMap;
  }

  /// Abre a tela de cálculo de CV% para comparação agronômica
  void _abrirCalculoCv() {
    if (_talhaoSelecionado == null) {
  ...
  void _gerarRelatorioQualidade() async {
    ...
    try {
      ...
      // Primeiro, tentar usar o serviço de integração de submódulos
      try {
        print('🔄 Tentando gerar relatório com dados integrados...');
        final relatorio = await _integrationService.gerarRelatorioComDadosSubmodulos(
          _talhaoSelecionado!.id,
          _culturaSelecionada?.id ?? _culturaManual,
        );

        if (relatorio != null) {
          print('✅ Relatório gerado com sucesso usando dados integrados');
        ...
        print('⚠️ Falha no serviço de integração: $e');
      }

      // Fallback: usar dados calculados diretamente
      print('🔄 Usando fallback com dados calculados...');
      
      if (_plantingCVModel == null || _estandePlantasModel == null) {
        throw Exception('Dados não disponíveis para gerar relatório');
      }

      final relatorio = await _plantingQualityReportService.gerarRelatorioComDadosReais(
        _plantingCVModel!,
        _estandePlantasModel!,
        _talhaoSelecionado!,
      );
  ...
  /// Abre a tela de histórico de calibrações
  void _abrirHistorico() {
    if (_talhaoSelecionado == null) {
      SnackbarUtils.showErrorSnackBar(context, 'Por favor, selecione um talhão primeiro');
      return;
    ...
    final culturaId = _culturaSelecionada?.id ?? _getCulturaIdFromName(_culturaManual);
    
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PlantioCalibragemHistoricoScreen(
