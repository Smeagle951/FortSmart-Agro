...
class AgricultureIntegrationMigration {
  final Database db;
  static const String _tag = 'AgricultureIntegrationMigration';
  ...
  Future<void> _addContextColumnsToTables() async {
    try {
      Logger.info('Iniciando adição de colunas de contexto agrícola às tabelas...', _tag);
      ...
      Logger.info('Colunas de contexto agrícola adicionadas com sucesso às tabelas.', _tag);
    } catch (e) {
      Logger.error('Erro ao adicionar colunas de contexto agrícola às tabelas', e, null, _tag);
      rethrow;
    }
  }
  ...
  Future<void> _addColumnIfNotExists(String table, String column, String type) async {
    try {
      ...
      if (!columnExists) {
        Logger.info('Adicionando coluna $column à tabela $table', _tag);
        await db.execute('ALTER TABLE $table ADD COLUMN $column $type');
      } else {
        Logger.info('Coluna $column já existe na tabela $table. Pulando...', _tag);
      }
    } catch (e) {
      Logger.error('Erro ao adicionar coluna $column à tabela $table', e, null, _tag);
      rethrow;
    }
  }
  ...
  Future<void> _backfillContextIds() async {
    try {
      Logger.info('Iniciando preenchimento automático de contexto agrícola...', _tag);
      ...
      for (final table in tables) {
        ...
        Logger.info('Encontrados ${registros.length} registros para preenchimento na tabela $table', _tag);
          ...
          if (talhaoId != null && talhaoId.isNotEmpty) {
            ...
            if (safras.isNotEmpty) {
              ...
              if (safraId != null) {
                ...
                Logger.info('Registro ${registro['id']} da tabela $table atualizado com safraId=$safraId, culturaId=$culturaId', _tag);
              }
            }
          }
        }
      ...
      Logger.info('Preenchimento automático de contexto agrícola concluído com sucesso.', _tag);
    } catch (e) {
      Logger.error('Erro ao preencher automaticamente dados de contexto agrícola', e, null, _tag);
      rethrow;
    }
  }
}
