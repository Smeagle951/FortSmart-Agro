...
class _InventoryReportScreenState extends State<InventoryReportScreen> with SingleTickerProviderStateMixin {
  ...
  void _calculateStatistics(List<InventoryItem> items) {
    ...
    for (var item in items) {
      ...
      final value = item.quantity * (item.unitPrice ?? 0);
  ...
  Widget _buildExpiringItemsList() {
    ...
    final expiringItems = _items.where((item) {
      ...
      try {
        // Converter a data de expiração para DateTime
        DateTime expirationDate;
        if (item.expirationDate is DateTime) {
          expirationDate = item.expirationDate as DateTime;
        } else if (item.expirationDate is String) {
          expirationDate = DateTime.parse(item.expirationDate as String);
        } else {
          return false;
        }
        
        // Verificar se está dentro do período de 30 dias
        return expirationDate.isAfter(now) && expirationDate.isBefore(thirtyDaysLater);
      } catch (e) {
        return false;
      }
    }).toList();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        try {
          if (item.expirationDate is DateTime) {
            formattedDate = DateFormat('dd/MM/yyyy').format(item.expirationDate as DateTime);
          } else if (item.expirationDate is String) {
            final date = DateTime.parse(item.expirationDate as String);
