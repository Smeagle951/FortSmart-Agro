...
class TalhaoProvider with ChangeNotifier {
  ...
  Future<void> addTalhao(TalhaoSafraModel talhao) async {
    try {
      _talhoes.add(talhao);
      notifyListeners();
    } catch (e) {
  ...
  /// Remove um talhão
  Future<void> removeTalhao(String talhaoId) async {
    try {
      _talhoes.removeWhere((talhao) => talhao.id == talhaoId);
      notifyListeners();
    } catch (e) {
      debugPrint('Erro ao remover talhão: $e');
      rethrow;
    }
  }

  /// Remove um talhão (alias para removeTalhao)
  Future<void> deleteTalhao(String talhaoId) async {
    await removeTalhao(talhaoId);
  }

  /// Atualiza um talhão existente
  Future<void> updateTalhao(TalhaoSafraModel talhaoAtualizado) async {
    try {
      final index = _talhoes.indexWhere((talhao) => talhao.id == talhaoAtualizado.id);
      if (index != -1) {
